...
//         address _promotionManagerAddress,
//         bytes32 gasLane,
...
//             promotionToTokenIDs[promotionID].push(initialID + i);
//         }
//         for (uint256 i = 0; i < numNotSeeded; i++) {
//             // Create a new seeded SeedReceipt for each ticket
//             seedReceipts[initialID + i + numSeeded] = SeedReceipt(
//                 promotionID,
//                 startParticipantNumber + i, // Increment the participant number for each ticket
//                 0, // Initialize to 0
//                 false,
...
//             // Set the receipt type
//             receiptTypes[initialID + i + numNotSeeded] = ReceiptType.Seed;

//             // Map the minted receipt ID to the promotion
//             promotionToTokenIDs[promotionID].push(initialID + numNotSeeded + i);
//         }

//         emit SeedReceiptsMinted(to, promotionID, startParticipantNumber, initialID, true, numSeeded);
//         emit SeedReceiptsMinted(to, promotionID, startParticipantNumber, initialID, true, numNotSeeded);
//     } 
...
//         address erc20Token;
//         ReceiptType firstReceiptType = receiptTypes[tokenIDs[0]];
//         //Get address of erc20 token of first receipt
//         if (firstReceiptType == ReceiptType.Snowball) {
//             erc20Token = snowballReceipts[tokenIDs[0]].erc20TokenAddress;
//         } else if (firstReceiptType == ReceiptType.Drawing) {
//             erc20Token = drawingReceipts[tokenIDs[0]].erc20TokenAddress;
//         } else if (firstReceiptType == ReceiptType.Seed) {
...
//                 SnowballReceipt storage snowballReceipt = snowballReceipts[tokenId];
//                 if (snowballReceipt.erc20TokenAddress != erc20Token) {
//                     revert("Different token type");
//                 }
//                 uint256 currentPrice = promotionManager.getSnowballPrice(
//                     snowballReceipt.promotionID
//                 );

//                 uint256 redeemable = snowballReceipt.effectivePricePaid - currentPrice;
//                 emit SnowballReceiptRedeemed(tokenId, snowballReceipt.promotionID, redeemable); 
//                 promotionManager.reduceCustodyBalance(
//                     snowballReceipt.promotionID,
//                     redeemable
//                 );

//                 snowballReceipt.effectivePricePaid = currentPrice;
...
//                 SeedReceipt memory seedReceipt = seedReceipts[tokenId];
...
//                     );
//                     seedReceipts[tokenId].redeemedAmount = redeemableAmount;
...
//         return promotionToTokenIDs[promotionID];
//     }
